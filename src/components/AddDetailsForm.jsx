...
const AddDetailsForm = () => {
  const router = useRouter();
  const [selectedAmenities, setSelectedAmenities] = useState([]);
  const [token, setToken] = useState(null); // Added state to save the token if needed

  // Move the token fetch safely inside a useEffect hook
  useEffect(() => {
    const fetchToken = async () => {
      try {
        ...
        console.error("Failed to fetch token on client:", error);
      }
    };
